...
export const ChatSidebar: React.FC = () => {
  ...
  return (
    <aside className="w-full h-[100dvh] bg-[#090A0F] border-r border-white/[0.08] flex flex-col select-none flex-shrink-0 z-20 font-sans shadow-2xl">
      {/* Top Header: Current User Profile + Action Tools */}
      <div className="p-3.5 sm:p-4 border-b border-white/[0.08] flex items-center justify-between bg-[#0F121C]/90 backdrop-blur-xl">
        {/* User Monogram & Info */}
        <button
        ...
        >
          <div className="relative">
            <Avatar
              ...
              size="md"
              status="online"
              className="ring-2 ring-white/10 group-hover:ring-white/30 transition shadow-lg"
            />
          </div>
      ...
      <div className="p-3 border-b border-white/[0.06] bg-[#0C0F17]/80 backdrop-blur-md space-y-2.5">
        {/* Action Buttons */}
        <div className="grid grid-cols-3 gap-1.5">
          ...
          <button
            onClick={() => setIsNewChatModalOpen(true)}
            className="py-2 px-2 bg-[#181C26] hover:bg-[#202634] active:scale-95 text-zinc-200 hover:text-white border border-white/[0.08] rounded-xl text-xs font-semibold shadow-sm flex items-center justify-center gap-1.5 transition"
          >
          ...
          <button
            onClick={() => setIsFriendsModalOpen(true)}
            className="py-2 px-2 bg-[#181C26] hover:bg-[#202634] active:scale-95 text-zinc-300 hover:text-white border border-white/[0.08] rounded-xl text-xs font-semibold shadow-sm flex items-center justify-center gap-1.5 transition relative"
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-black/50 border border-white/[0.08] rounded-xl text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500/50 focus:bg-black/80 transition"
          />
          {searchQuery && (
            <button
          ...
          {[
            ...
            <button
              ...
              className={`flex-1 py-1 px-2 rounded-lg text-xs font-semibold transition ${
                filterTab === tab.id
                  ? 'bg-[#1D2230] text-white shadow-sm border border-white/10'
            ...
            return (
              <button
                ...
                className={`w-full p-3 rounded-2xl flex items-center gap-3 transition-all text-left group relative ${
                  isSelected
                    ? 'bg-gradient-to-r from-blue-600/15 via-indigo-600/10 to-transparent border-l-2 border-indigo-500 text-white shadow-lg shadow-black/40'
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-1">
                    ...
                    {conv.unreadCount > 0 && (
                      <span className="px-1.5 min-w-4 h-4 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-black flex items-center justify-center flex-shrink-0 shadow-md">
                        {conv.unreadCount}
                      </span>
                    )}
                  </div>
